    ...
    >
      ...
      <div class="flex flex-col gap-1">
        ...
        <p class="text-p-sm text-ink-gray-6">
          {{
            __(
              'Quét/nhập mã vật tư, kiểm CO/CQ + FIFO, rồi lập phiếu xuất kho.',
            )
          }}
        </p>
      </div>
    ...
    <section class="flex-1 overflow-auto px-6 py-5">
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div
            class="flex flex-col gap-4 rounded-lg border border-outline-gray-modals bg-surface-white p-4 sm:flex-row"
          >
          ...
          >
            {{
              __(
                '⚠ {0} thiếu CO/CQ — không thể xuất. Gọi Pháp lý đính file trước.',
                ...
                >
                  ...
                  >
                    <input
                      ...
                      :aria-label="
                        __('Số lượng xuất {0}', [row.item_name || row.item])
                      "
                      @input="(e) => setQty(idx, e.target.value)"
                    />
                  </td>
                  <td
                    class="border-b border-outline-gray-1 px-3 py-2.5 text-right tabular-nums text-ink-gray-8"
                  >
                  ...
                  <td class="border-b border-outline-gray-1 px-3 py-2.5">
                    <button
                      ...
                      :aria-label="
                        __('Xoá dòng {0}', [row.item_name || row.item])
                      "
                      @click="removeRow(idx)"
                    >
                      ✕
        ...
        <aside class="flex flex-col gap-4">
          <div
            class="flex flex-col gap-3 rounded-lg border border-outline-gray-modals bg-surface-white p-4"
          >
          ...
          >
            ...
            <div
              v-if="recent.loading"
              class="py-4 text-center text-p-xs text-ink-gray-5"
            >
              {{ __('Đang tải…') }}
            </div>
            <div
              v-else-if="recent.error"
              class="py-4 text-center text-p-xs text-red-600"
            >
              {{ __('Không tải được.') }}
            </div>
            <p
            ...
            <ul v-else class="flex flex-col divide-y divide-outline-gray-1">
              ...
              >
                ...
                <span class="text-p-xs tabular-nums text-ink-gray-5">{{
                  formatStockTime(p.posting_datetime)
                }}</span>
              </li>
            </ul>
          </div>
...
const warehouses = listWarehouses({
  params: { warehouse_type: 'Tổng', page_length: 100 },
  auto: true,
})
...
employees.onError = (err) =>
  toast.error(err?.messages?.[0] || __('Không tải được danh sách NV'))

// Quét/nhập mã → scan_lot (RAW dict THƯỜNG, đọc r.data.* trực tiếp).
const scan = scanLot()

// Phiếu xuất gần đây (dict bọc → r.data.data).
const recent = listStockEntries({
  params: { entry_type: 'Xuất cho NV', start: 0, page_length: 8 },
  auto: true,
})
...
const totalValue = computed(() =>
  rows.value.reduce((sum, r) => sum + lineValue(r), 0),
)
...
function addScanned(data) {
  ...
  if (!data || !data.found) {
    if (data?.reason === 'no_perm')
      toast.error(__('Bạn không có quyền đọc lô này.'))
  ...
  if (data.recall_status === 'Đã thu hồi') {
    toast.error(
      __('Lô {0} đã bị thu hồi — không thể xuất.', [data.lot_no || data.lot]),
    )
    return
  }
  ...
  if (data.days_to_expiry != null && data.days_to_expiry < 0) {
    toast.error(
      __('Lô {0} đã hết hạn — không thể xuất.', [data.lot_no || data.lot]),
    )
    return
  }
...
function doScan(code) {
  ...
  scan.submit(
    { code: code.trim(), warehouse: fromWarehouse.value },
    {
      ...
      onError: (err) =>
        toast.error(err?.messages?.[0] || __('Không quét được mã')),
    },
  )
}
...
function submitIssue() {
  ...
  entry.submit(
    ...
    },
    {
      ...
      onError: (err) =>
        toast.error(err?.messages?.[0] || __('Không xuất được phiếu')),
    },
  )
}
</script>
