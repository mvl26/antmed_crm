  ...
  <main class="flex h-full flex-col" aria-labelledby="antmed-checklist-title">
    ...
    >
      <nav class="text-p-xs text-ink-gray-5" :aria-label="__('Đường dẫn')">
        ...
        <RouterLink
          ...
          class="rounded text-ink-gray-6 hover:text-ink-gray-8"
          >{{ __('Bộ dụng cụ') }}</RouterLink
        >
        <span class="px-1.5 text-ink-gray-4" aria-hidden="true">›</span>
        <span class="text-ink-gray-7" aria-current="page"
          >{{ __('Checklist') }} {{ name }}</span
        >
      </nav>
      <div class="flex flex-wrap items-center gap-3">
        <h1
          id="antmed-checklist-title"
          class="text-xl font-semibold text-ink-gray-9"
        >
          🧰 {{ loan.set_code || loan.instrument_set || '—' }}
          <span v-if="loan.surgery_type" class="text-ink-gray-6"
            >· {{ loan.surgery_type }}</span
          >
        </h1>
        <Badge
          v-if="loan.status"
          variant="subtle"
          :theme="statusTheme(loan.status)"
          size="md"
          :label="loan.status"
        />
        <Badge
    ...
    <section class="flex-1 overflow-auto px-6 pb-8 pt-4" aria-live="polite">
      ...
      <template v-else>
        <!-- Bối cảnh ca mổ -->
        <div
          class="mb-5 rounded-lg border border-outline-gray-2 bg-surface-white p-4"
        >
          ...
          <p class="mt-1 text-p-sm text-ink-gray-6">
            <template v-if="loan.hospital_name"
              >🏥 {{ loan.hospital_name }}</template
            >
            <template v-if="loan.doctor_name">
              · 👨‍⚕️ {{ loan.doctor_name }}</template
            >
            <template v-if="loan.employee_name">
              · NV: {{ loan.employee_name }}</template
            >
          </p>
          <p class="mt-1 text-p-xs text-ink-gray-5">
            {{ __('Đặt') }}: {{ fmtDate(loan.booked_at) }} ·
            {{ __('Hẹn trả') }}: {{ fmtDate(loan.due_return_at) }}
          </p>
        </div>
        ...
        >
          <div class="mb-3 flex items-center justify-between gap-2">
            <h2 class="text-p-base font-semibold text-ink-gray-8">
              {{ __('Checklist bộ dụng cụ') }} ·
              {{ __('đủ {0}/{1} món', [okCount, editItems.length]) }}
            </h2>
            <Button
              v-if="canEdit"
              variant="subtle"
          ...
          <p v-if="!editItems.length" class="text-p-sm text-ink-gray-5">
            {{ __('Lượt mượn này chưa có danh mục món.') }}
          </p>
          ...
          <ul v-else class="flex flex-col divide-y divide-outline-gray-1">
            ...
            >
              <span class="min-w-0 flex-1 text-p-base text-ink-gray-8">
                {{ it.component_name }}
                <span class="text-p-xs text-ink-gray-4"
                  >× {{ it.expected }}</span
                >
              </span>
              <span
                v-if="it.photo"
                class="text-p-xs text-teal-600"
                :title="__('Có ảnh đính kèm')"
                >📷</span
              >
              ...
              <label v-if="canEdit" class="flex items-center gap-1.5">
                <span class="sr-only">{{
                  __('Tình trạng món {0}', [it.component_name])
                }}</span>
                <select
        ...
        <p v-else class="mt-5 text-p-sm text-ink-gray-5">
          {{ __('Lượt mượn đã khép vòng đời — không còn thao tác.') }}
        </p>
      </template>
    </section>
  </main>
...
resource.onError = (err) =>
  toast.error(err?.messages?.[0] || __('Không tải được lượt mượn'))
...
const okCount = computed(
  () => editItems.value.filter((it) => it.condition === 'OK').length,
)
// Sửa được khi lượt mới ở 'Đã đặt' và chưa submit (sau bàn giao → khoá).
const canEdit = computed(
  () => loan.value.status === 'Đã đặt' && loan.value.docstatus === 0,
)
...
const saveChecklist = createResource({
  ...
  onError: (err) =>
    toast.error(err?.messages?.[0] || __('Lưu checklist thất bại')),
})
function onSave() {
...
function makeAction(fn, successMsg, extra = {}) {
  const r = createResource({
    ...
    onError: (err) =>
      toast.error(err?.messages?.[0] || __('Thao tác thất bại')),
  })
...
const sterilize = makeAction('sterilize', __('Đã ghi nhận tiệt khuẩn'), {
  method: 'Hấp',
  result: 'Pass',
})
const markReady = makeAction(
  'mark_ready',
  __('Bộ đã sẵn sàng cho lượt mượn kế'),
)

const ACTION_BY_STATUS = {
  'Đã đặt': {
    a: handover,
    label: __('Hoàn tất bàn giao mượn'),
    hint: __('Ký nhận & chuyển sang Đang sử dụng tại BV'),
  },
  'Đang sử dụng tại BV': {
    a: receiveReturn,
    label: __('Nhận bộ về kho NV'),
    hint: __('Sau ca mổ — chuyển sang Đã trả về NV KD'),
  },
  'Đã trả về NV KD': {
    a: sterilize,
    label: __('Gửi tiệt khuẩn (Hấp)'),
    hint: __('Ghi nhận tiệt khuẩn để chuẩn bị sẵn sàng'),
  },
  'Đang xử lý/tiệt khuẩn': {
    a: markReady,
    label: __('Đánh dấu sẵn sàng'),
    hint: __('Yêu cầu có kết quả tiệt khuẩn Pass (BR-09)'),
  },
}
