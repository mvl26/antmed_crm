<template>
  <main
    ...
    aria-labelledby="antmed-stock-receipt-title"
  >
    ...
    >
      ...
      <div class="flex flex-col gap-1">
        ...
        <p class="text-p-sm text-ink-gray-6">
          {{
            __(
              'Quét/nhập mã vật tư, nhập số lượng + đơn giá, rồi lập phiếu nhập kho.',
            )
          }}
        </p>
      </div>
    ...
    <section class="flex-1 overflow-auto px-6 py-5">
      <div class="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div class="flex flex-col gap-4">
          <div
            class="flex flex-col gap-1 rounded-lg border border-outline-gray-modals bg-surface-white p-4 sm:max-w-md"
          >
            <label
              for="sr-warehouse"
                ...
                >
                  ...
                  >
                    <input
                      ...
                      :aria-label="
                        __('Số lượng nhập {0}', [row.item_name || row.item])
                      "
                  ...
                  >
                    <input
                      ...
                      :aria-label="
                        __('Đơn giá {0}', [row.item_name || row.item])
                      "
            ...
            <div class="flex flex-col gap-1">
              ...
              <form class="flex gap-2" @submit.prevent="onManualEnter">
                ...
                <Button
                  ...
                  :disabled="!manualCode.trim() || !toWarehouse"
                />
              </form>
              <p v-if="!toWarehouse" class="text-p-xs text-amber-700">
                {{ __('Chọn kho đến trước khi quét.') }}
              </p>
            </div>
          </div>
        </aside>
...
const totalValue = computed(() =>
  rows.value.reduce((s, r) => s + lineValue(r), 0),
)
...
function submitReceipt() {
  ...
  entry.submit(
    ...
    },
    {
      ...
      onError: (err) =>
        toast.error(err?.messages?.[0] || __('Không nhập được phiếu')),
    },
  )
}
</script>
