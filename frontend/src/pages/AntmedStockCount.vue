    ...
    >
      <div class="flex flex-col gap-2">
        <nav class="text-p-xs text-ink-gray-5" :aria-label="__('Đường dẫn')">
          ...
          <span class="px-1.5 text-ink-gray-4" aria-hidden="true">›</span>
          <span class="text-ink-gray-7" aria-current="page">{{ __('Kiểm kê') }}</span>
        </nav>
        <div class="flex flex-col gap-1">
          <h1 id="antmed-stock-count-title" class="text-xl font-semibold text-ink-gray-9">
            {{ __('Kiểm kê kho') }}
          </h1>
          <p class="text-p-sm text-ink-gray-6">
            {{ __('Đối chiếu tồn hệ thống với số thực đếm; chốt để điều chỉnh sổ tồn.') }}
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1 sm:w-72">
        <label for="sc-warehouse" class="text-p-xs font-medium text-ink-gray-6">{{ __('Kho') }}</label>
        <FormControl
          id="sc-warehouse"
          type="select"
      ...
      <div class="flex flex-col gap-5">
        ...
        <div class="flex flex-col gap-3">
          ...
          >
            ...
            <p class="text-p-sm">{{ __('Chọn một kho ở trên để tải tồn hệ thống.') }}</p>
          </div>

          <div v-else-if="snapshot.loading" class="flex items-center justify-center gap-2 py-16 text-ink-gray-6">
            <LoadingIndicator class="h-4 w-4" />
            <span class="text-p-base">{{ __('Đang tải tồn kho…') }}</span>
          </div>

          <div v-else-if="snapshot.error" class="flex flex-col items-center gap-3 py-16 text-center" role="alert">
            <Badge variant="subtle" theme="red" size="lg" :label="__('Không tải được')" />
            <p class="max-w-md text-p-sm text-ink-gray-6">{{ snapshotError }}</p>
            <Button variant="outline" :label="__('Thử lại')" @click="snapshot.reload()" />
          </div>
          ...
          >
            ...
            <p class="text-p-sm">{{ __('Kho này không có vật tư còn tồn để kiểm kê.') }}</p>
          </div>
          ...
          <template v-else>
            <div class="overflow-x-auto rounded-lg border border-outline-gray-modals bg-surface-white">
              <table class="w-full border-separate border-spacing-0 text-left">
                <caption class="sr-only">{{ __('Bảng kiểm kê tồn kho') }}</caption>
                <thead>
                  <tr class="text-p-xs text-ink-gray-6">
                    ...
                    <th class="border-b border-outline-gray-1 px-3 py-2 text-right font-medium">{{ __('SL hệ thống') }}</th>
                    <th class="border-b border-outline-gray-1 px-3 py-2 text-right font-medium">{{ __('SL thực đếm') }}</th>
                    <th class="border-b border-outline-gray-1 px-3 py-2 text-right font-medium">{{ __('Chênh lệch') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(row, idx) in rows" :key="`${row.item}-${row.lot}-${idx}`" class="text-p-sm text-ink-gray-8">
                    ...
                    <td class="border-b border-outline-gray-1 px-3 py-2.5 text-right tabular-nums text-ink-gray-7">{{ fmtQty(row.system_qty) }}</td>
                    <td class="border-b border-outline-gray-1 px-3 py-2.5 text-right">
                      <input
                        type="number" step="1" :value="row.counted_qty"
                        ...
                        :aria-label="__('Số lượng thực đếm {0}', [row.item_name || row.item])"
                        @input="(e) => setCounted(idx, e.target.value)"
                      />
                    </td>
                    <td class="border-b border-outline-gray-1 px-3 py-2.5 text-right tabular-nums font-medium" :class="varianceTextClass(variance(row))">
                      {{ variance(row) > 0 ? '+' : '' }}{{ fmtQty(variance(row)) }}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div class="flex flex-1 flex-col gap-1 sm:max-w-md">
                <label for="sc-note" class="text-p-xs font-medium text-ink-gray-6">{{ __('Ghi chú') }}</label>
                <FormControl id="sc-note" v-model="note" type="text" :placeholder="__('Lý do kiểm kê (tuỳ chọn)')" />
              </div>
              <div class="flex items-center gap-3">
                <p class="text-p-sm text-ink-gray-5">
                  {{ __('Tổng chênh') }}: <span class="font-medium" :class="varianceTextClass(totalVariance)">{{ totalVariance > 0 ? '+' : '' }}{{ fmtQty(totalVariance) }}</span>
                </p>
                <Button
                  variant="solid"
        ...
        <div class="flex flex-col gap-2 rounded-lg border border-outline-gray-modals bg-surface-white p-4">
          <h2 class="text-p-sm font-semibold text-ink-gray-8">{{ __('Lịch sử kiểm kê') }}</h2>
          <div v-if="history.loading" class="py-3 text-center text-p-xs text-ink-gray-5">{{ __('Đang tải…') }}</div>
          <div v-else-if="history.error" class="py-3 text-center text-p-xs text-red-600">{{ __('Không tải được.') }}</div>
          <p v-else-if="!historyRows.length" class="py-3 text-center text-p-xs text-ink-gray-5">{{ __('Chưa có phiếu kiểm kê.') }}</p>
          <table v-else class="w-full border-separate border-spacing-0 text-left">
            <caption class="sr-only">{{ __('Bảng lịch sử phiếu kiểm kê') }}</caption>
            <thead>
              <tr class="text-p-xs text-ink-gray-6">
                <th class="border-b border-outline-gray-1 py-2 pr-4 font-medium">{{ __('Số phiếu') }}</th>
                <th class="border-b border-outline-gray-1 py-2 pr-4 font-medium">{{ __('Người đếm') }}</th>
                <th class="border-b border-outline-gray-1 py-2 pr-4 font-medium">{{ __('Lúc') }}</th>
                <th class="border-b border-outline-gray-1 py-2 pr-4 text-right font-medium">{{ __('Tổng chênh') }}</th>
                <th class="border-b border-outline-gray-1 py-2 font-medium">{{ __('Trạng thái') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="h in historyRows" :key="h.name" class="text-p-sm text-ink-gray-8">
                <td class="border-b border-outline-gray-1 py-2.5 pr-4 font-medium">{{ h.name }}</td>
                <td class="border-b border-outline-gray-1 py-2.5 pr-4 text-ink-gray-7">{{ h.counted_by_name || h.counted_by || '—' }}</td>
                <td class="border-b border-outline-gray-1 py-2.5 pr-4 tabular-nums text-ink-gray-7">{{ formatStockTime(h.count_datetime) }}</td>
                <td class="border-b border-outline-gray-1 py-2.5 pr-4 text-right tabular-nums" :class="varianceTextClass(h.total_variance_qty)">
                  {{ Number(h.total_variance_qty) > 0 ? '+' : '' }}{{ fmtQty(h.total_variance_qty) }}
                </td>
                <td class="border-b border-outline-gray-1 py-2.5">
                  <Badge variant="subtle" size="sm" :theme="docstatusTheme(h.docstatus)" :label="docstatusLabel(h.docstatus)" />
                </td>
              </tr>
            </tbody>
          </table>
...
import { listWarehouses, stockCountSnapshot, createStockCount, listStockCounts } from '@/data/antmed'
import {
  fmtDate,
  fmtQty,
...
const snapshotError = computed(
  () => snapshot.error?.messages?.[0] || snapshot.error?.message || __('Không tải được tồn kho'),
)
...
const totalVariance = computed(() => rows.value.reduce((s, r) => s + variance(r), 0))
  ...
  snapshot.submit(
    { warehouse },
    {
      onSuccess: (data) => {
        ...
        rows.value = (data?.rows || []).map((r) => ({ ...r, counted_qty: r.system_qty }))
      },
      onError: (err) => toast.error(err?.messages?.[0] || __('Không tải được tồn kho')),
    },
  )
  ...
  counting.submit(
    {
      ...
      items: rows.value.map((r) => ({ item: r.item, lot: r.lot, counted_qty: Number(r.counted_qty) })),
    },
    {
      onSuccess: (data) => {
      ...
      onError: (err) => toast.error(err?.messages?.[0] || __('Không chốt được kiểm kê')),
    },
  )
}
</script>
