    ...
    >
      ...
      <div class="flex flex-wrap items-center gap-3">
        <h1 id="antmed-set-title" class="text-xl font-semibold text-ink-gray-9">
          🧰 {{ set.set_code || name }}
          <span v-if="set.surgery_type" class="text-ink-gray-6"
            >· {{ set.surgery_type }}</span
          >
        </h1>
        <Badge
      ...
      <template v-else>
        ...
        <dl class="mb-6 grid grid-cols-2 gap-x-6 gap-y-3 sm:grid-cols-3">
          <div>
            ...
            <dd class="text-p-base font-medium text-ink-gray-9 tabular-nums">
              {{ components.length }}
            </dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd class="text-p-base font-medium text-ink-gray-9">
              {{ set.asset_value ? formatVnMoney(set.asset_value) : '—' }}
            </dd>
          </div>
          <div>
            <dt class="text-p-xs uppercase text-ink-gray-5">
        ...
        >
          <h2 class="mb-3 text-p-base font-semibold text-ink-gray-8">
            {{ __('Danh mục món') }} ({{ components.length }})
          </h2>
          <p v-if="!components.length" class="text-p-sm text-ink-gray-5">
            {{ __('Chưa khai báo món nào cho bộ này.') }}
          </p>
          <table
            v-else
          ...
          >
            ...
            <tbody>
              ...
              >
                ...
                <td
                  class="border-b border-outline-gray-1 py-2 pr-4 text-right tabular-nums"
                >
                  {{ c.qty }}
                </td>
        ...
        >
          <h2 class="mb-3 text-p-base font-semibold text-ink-gray-8">
            {{ __('Lịch sử lượt mượn') }}
          </h2>
          <p v-if="!loans.length" class="text-p-sm text-ink-gray-5">
            {{ __('Chưa có lượt mượn nào.') }}
          </p>
          <ul v-else class="flex flex-col gap-2">
            <li v-for="ln in loans" :key="ln.name">
              ...
              >
                ...
                <Badge
                  variant="subtle"
                  :theme="statusTheme(ln.status)"
                  size="sm"
                  :label="ln.status"
                />
                <span class="text-p-xs text-ink-gray-5"
                  >{{ __('Đặt') }}: {{ fmtDate(ln.booked_at) }}</span
                >
                <span v-if="ln.returned_at" class="text-p-xs text-ink-gray-5"
                  >· {{ __('Trả') }}: {{ fmtDate(ln.returned_at) }}</span
                >
              </RouterLink>
            </li>
          </ul>
...
resource.onError = (err) =>
  toast.error(err?.messages?.[0] || __('Không tải được bộ dụng cụ'))
