<template>
  <main class="flex h-full flex-col" aria-labelledby="antmed-tender-title">
    ...
    >
      <nav class="text-p-xs text-ink-gray-5" :aria-label="__('Đường dẫn')">
        ...
        <RouterLink
          to="/antmed/tenders"
          class="rounded text-ink-gray-6 hover:text-ink-gray-8"
          >{{ __('Pipeline gói thầu') }}</RouterLink
        >
        <span class="px-1.5 text-ink-gray-4" aria-hidden="true">›</span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        />
        <Badge
          v-if="d.win_probability_pct != null"
          variant="subtle"
          theme="blue"
          size="md"
          :label="d.win_probability_pct + '%'"
        />
      </div>
    </header>
    ...
    <section class="flex-1 overflow-auto px-6 pb-8 pt-5" aria-live="polite">
      ...
      >
        <Badge
          ...
          theme="red"
          size="lg"
          :label="__('Không tải được gói thầu')"
        />
      ...
      <div v-else class="mx-auto max-w-3xl">
        ...
        <dl
          class="mt-4 grid grid-cols-1 gap-x-8 gap-y-4 rounded-xl border border-outline-gray-2 bg-surface-white p-5 sm:grid-cols-2"
        >
          <div>
          ...
          <div>
            ...
            <dd class="text-p-base font-semibold text-ink-gray-9">
              {{
                d.estimated_value
                  ? formatVnMoney(d.estimated_value) + ' ₫'
                  : '—'
              }}
            </dd>
          </div>
          <div>
            ...
            <dd class="text-p-base text-ink-gray-9">
              {{
                d.win_probability_pct != null
                  ? d.win_probability_pct + '%'
                  : '—'
              }}
            </dd>
          </div>
          ...
          <div v-if="d.result">
            ...
            <dd>
              <Badge
                variant="subtle"
                :theme="d.result === 'Trúng' ? 'green' : 'red'"
                size="sm"
                :label="d.result"
              />
            </dd>
        ...
        >
          ...
          <div class="flex flex-col gap-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <span v-else class="text-p-sm text-ink-gray-4">{{
                __('Chưa có')
              }}</span>
            </div>
            <div class="flex items-center justify-between gap-2">
              ...
              <span
                v-if="d.deal"
                class="text-p-sm font-medium text-ink-gray-8"
                >{{ d.deal }}</span
              >
              <span v-else class="text-p-sm text-ink-gray-4">{{
                __('Chưa gắn deal')
        ...
        <div
          v-if="d.status === 'Dự thầu'"
          class="mt-5 flex flex-wrap items-center gap-3"
        >
          <Button
            variant="solid"
            theme="teal"
            :label="__('Chốt kết quả thầu')"
            @click="resultDlg = true"
          />
          <span class="text-p-xs text-ink-gray-5">{{
            __('Trúng (kèm số QĐ → tự tạo HĐ nháp) hoặc Trượt')
          }}</span>
        </div>
      </div>
    </section>
    ...
    <Dialog
      v-model="resultDlg"
      :options="{ title: __('Chốt kết quả: {0}', [d.tender_no]) }"
    >
      <template #body-content>
        <div class="bg-surface-modal px-4 pb-6 pt-5 sm:px-6">
          <div class="flex flex-col gap-3">
            <label class="flex items-center gap-2 text-p-base">
              <input v-model="rResult" type="radio" value="Trúng" />
              <Badge
                variant="subtle"
                theme="green"
                size="sm"
            ...
            <label class="flex items-center gap-2 text-p-base">
              <input v-model="rResult" type="radio" value="Trượt" />
              <Badge
                variant="subtle"
                theme="red"
                size="sm"
...
resource.onError = (err) =>
  toast.error(err?.messages?.[0] || __('Không tải được gói thầu'))
...
function onResult() {
  resultRes.submit({
    ...
    decision_no: rDecision.value || undefined,
  })
}
</script>
