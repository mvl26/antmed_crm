<template>
  <main class="flex h-full flex-col" aria-labelledby="antmed-pipeline-title">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p class="text-p-sm text-ink-gray-6">
            {{
              __(
                'Kéo thả thẻ giữa các cột để đổi giai đoạn (2 chiều). Thả vào “Trúng/Trượt” để chốt kết quả.',
              )
            }}
          </p>
        </div>
    ...
    >
      <div>
        ...
        <div class="text-lg font-bold tabular-nums text-teal-700">
          {{ formatVnMoney(forecast.data?.total_weighted || 0) }} ₫
        </div>
      </div>
    ...
    <section class="flex-1 overflow-auto p-4" aria-live="polite">
      ...
      <div v-else class="flex gap-3 overflow-x-auto pb-2">
        <div
          ...
          :class="
            dragOverStage === stage ? 'ring-2 ring-teal-400 ring-offset-1' : ''
          "
          ...
          <div class="flex min-h-[80px] flex-1 flex-col gap-2">
            ...
            >
              <div class="flex items-start justify-between gap-2">
                <span class="text-p-xs font-medium text-teal-700">{{
                  t.tender_no
                }}</span>
                <Badge
                  variant="subtle"
                  ...
                  size="sm"
                  :label="
                    t.win_probability_pct != null
                      ? t.win_probability_pct + '%'
                      : '—'
                  "
                />
              </div>
              <p class="mt-0.5 text-p-sm font-semibold text-ink-gray-9">
                {{ t.tender_name }}
              </p>
              ...
              <p
                class="mt-1 text-p-sm font-medium tabular-nums text-ink-gray-8"
              >
                {{ formatVnMoney(t.estimated_value || 0) }} ₫
              </p>
              <p
                v-if="t.status === 'Trúng' && t.decision_no"
                class="mt-1 text-p-xs font-medium text-green-700"
              >
                {{ __('QĐ') }}: {{ t.decision_no }}
              </p>
    ...
    <Dialog
      v-model="createDlg"
      :options="{ title: __('Tạo gói thầu mới'), size: 'xl' }"
    >
    ...
    <Dialog
      v-model="resultDlg"
      :options="{ title: __('Chốt kết quả: {0}', [rForm.tender_no]) }"
    >
      <template #body-content>
        <div class="bg-surface-modal px-4 pb-6 pt-5 sm:px-6">
          <div class="flex flex-col gap-3">
            <label class="flex items-center gap-2 text-p-base">
              <input v-model="rForm.result" type="radio" value="Trúng" />
              <Badge
                variant="subtle"
                theme="green"
                size="sm"
            ...
            <label class="flex items-center gap-2 text-p-base">
              <input v-model="rForm.result" type="radio" value="Trượt" />
              <Badge
                variant="subtle"
                theme="red"
                size="sm"
            ...
            >
              {{
                __(
                  'BR-M08: Trúng bắt buộc có số quyết định; hệ thống tự tạo HĐ nháp.',
                )
              }}
            </p>
          </div>
...
tenders.onError = (err) =>
  toast.error(err?.messages?.[0] || __('Không tải được pipeline'))
...
const resultRes = createResource({
  ...
  onError: (err) =>
    toast.error(err?.messages?.[0] || __('Không chốt được kết quả')),
})
...
function openCreate() {
  cForm.value = {
    tender_no: '',
    tender_name: '',
    hospital: hospitalOptions.value[0]?.value || '',
    estimated_value: null,
    source: '',
    bid_open_date: '',
    bid_close_date: '',
...
function onResult() {
  resultRes.submit({
    ...
    decision_no: rForm.value.decision_no || undefined,
  })
}
</script>
