  ...
  <main class="flex h-full flex-col" aria-labelledby="antmed-profile-title">
    ...
    <section class="flex-1 overflow-auto px-6 pb-8 pt-5">
      ...
      <div v-else class="mx-auto max-w-2xl">
        ...
        <div
          class="flex items-center gap-4 rounded-2xl bg-gradient-to-br from-teal-600 to-teal-800 p-5 text-white shadow-sm ring-1 ring-teal-900/10"
        >
          <div
            class="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-white/20 text-xl font-bold text-white ring-2 ring-white/40"
          >
            {{ initials }}
          </div>
          <div class="min-w-0">
            ...
            <div class="mt-2 flex flex-wrap gap-1">
              ...
              />
              <Badge
                :theme="me.enabled ? 'green' : 'red'"
                variant="subtle"
                size="sm"
                :label="me.enabled ? __('Active') : __('Khoá')"
              />
            </div>
          </div>
        </div>
        ...
        <!-- Thông tin chi tiết -->
        <dl
          class="mt-5 grid grid-cols-1 gap-x-8 gap-y-4 rounded-xl border border-outline-gray-2 bg-surface-white p-5 sm:grid-cols-2"
        >
          <div>
          ...
          <div>
            ...
            <dd>
              <Badge
                variant="subtle"
                :theme="me.two_factor ? 'green' : 'gray'"
                size="sm"
                :label="me.two_factor ? __('Bật') : __('Tắt')"
              />
            </dd>
          </div>
          <div>
...
const profile = getMyProfile({
  auto: true,
  onError: (err) =>
    toast.error(err?.messages?.[0] || __('Không tải được hồ sơ')),
})
