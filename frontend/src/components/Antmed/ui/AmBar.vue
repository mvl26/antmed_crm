<template>
  ...
  <div class="flex items-center gap-1.5">
    <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-ink-gray-2">
      <div class="h-full rounded-full" :class="barFillClass(theme)" :style="{ width: clamped + '%' }" />
    </div>
    <span v-if="showLabel" class="text-[10px] tabular-nums text-ink-gray-6">{{ clamped }}%</span>
  </div>
</template>
