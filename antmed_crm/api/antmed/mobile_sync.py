...
def bootstrap() -> dict:
	...
	doctors = frappe.get_list(
		"AntMed Doctor", fields=["name", "full_name", "hospital", "specialty"], limit_page_length=0
	)
...
def scan_lot(code: str) -> dict:
	...
	lot = frappe.db.get_value(
		LOT_DOCTYPE,
		code,
		["name", "item", "expiry_date", "recall_status", "co_cert", "cq_cert"],
		as_dict=True,
	)
	if not lot:
...
def register_device(
	device_id: str, push_token: str | None = None, platform: str | None = None, app_version: str | None = None
) -> dict:
	...
	if frappe.db.exists(DEVICE_DOCTYPE, device_id):
		...
		frappe.get_doc({"doctype": DEVICE_DOCTYPE, "device_id": device_id, **values}).insert(
...
def _outbox_done(key: str | None) -> bool:
	...
	return bool(key) and bool(
		frappe.db.exists(SYNC_LOG_DOCTYPE, {"doctype_synced": key, "direction": "Push", "status": "OK"})
	)
