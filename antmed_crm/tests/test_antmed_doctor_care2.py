...
class TestAntMedDoctorCare2(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		cls.doctor = _ensure("AntMed Doctor", "doctor_code", "_T-CARE2-BS", {"full_name": "BS Care2", "hospital": cls.hosp})
	...
	def test_call_plan_today(self):
		frappe.get_doc({"doctype": "AntMed Call Plan", "doctor": self.doctor, "sales_rep": "Administrator", "next_visit": nowdate()}).insert(ignore_permissions=True)
	...
	def test_doctor_birthdays(self):
		...
		bs = _ensure("AntMed Doctor", "doctor_code", "_T-CARE2-BDAY", {"full_name": "BS Sinh Nhật", "hospital": self.hosp, "birthday": f"1980-{bday[5:7]}-{bday[8:10]}"})
