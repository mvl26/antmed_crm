...
class TestAntMedMaterialRequest(FrappeTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.hosp = _ensure("AntMed Hospital", "hospital_code", "_T-MR-BV", {"hospital_name": "BV Material Req"})
		cls.doctor = _ensure("AntMed Doctor", "doctor_code", "_T-MR-BS", {"full_name": "BS Gọi VT", "hospital": cls.hosp})
			...
			c = frappe.get_doc(
				{
					...
					"status": "Hiệu lực",
					"valid_to": "2026-12-31",
					"items": [{"item": cls.item_in, "item_name": "Chỉ Vicryl MR", "uom": "Cái", "unit_price": 100000, "quota_qty": 100, "used_qty": 0, "remaining_pct": 100, "lock_at_100": 1}],
				}
			)
