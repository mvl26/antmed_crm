...
class TestRecallNotification(FrappeTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.item = _mk("AntMed Item", "_T-RCN-ITEM", {"item_code": "_T-RCN-ITEM", "item_name": "VT recall notif", "requires_cocq": 0}).name
		# BV1 giữ ký gửi; BV2 đã được giao.
		cls.bv1 = _mk("AntMed Hospital", "_T-RCN-BV1", {"hospital_code": "_T-RCN-BV1", "hospital_name": "BV Ký Gửi Recall"}).name
		cls.bv2 = _mk("AntMed Hospital", "_T-RCN-BV2", {"hospital_code": "_T-RCN-BV2", "hospital_name": "BV Giao Recall"}).name
		cls.cg_wh = _mk("AntMed Warehouse", "_T-RCN-CG", {"warehouse_name": "_T-RCN-CG", "warehouse_type": "Ký gửi BV", "hospital": cls.bv1}).name
		# Thủ kho nhận thông báo.
		cls.thukho = "_t_rcn_thukho@example.com"
		if not frappe.db.exists("User", cls.thukho):
			u = frappe.get_doc({"doctype": "User", "email": cls.thukho, "first_name": "Thủ kho Recall", "send_welcome_email": 0})
			u.insert(ignore_permissions=True)
			u.add_roles("Thủ kho")

	def _fresh_lot(self, suffix):
		"""Lô mới (Bình thường, còn hạn) + ký gửi BV1 20 + giao BV2 10 → 2 BV ảnh hưởng."""
		lot = _mk("AntMed Lot", f"_T-RCN-LOT-{suffix}", {"lot_no": f"_T-RCN-LOT-{suffix}", "item": self.item, "expiry_date": add_days(nowdate(), 300)}).name
		inventory.create_stock_entry(entry_type="Nhập ký gửi BV", to_warehouse=self.cg_wh, hospital=self.bv1, items=[{"item": self.item, "lot": lot, "qty": 20}])
		frappe.get_doc(
			{"doctype": "AntMed Delivery", "hospital": self.bv2, "surgery_datetime": f"{add_days(nowdate(), 1)} 09:00:00",
			 "status": "Nháp", "items": [{"item": self.item, "lot": lot, "requested_qty": 10, "consumed_qty": 10}]}
	...
	def test_recall_notifies_thukho(self):
		...
		self.assertTrue(frappe.db.exists("Notification Log", {"for_user": self.thukho}), "Thủ kho phải nhận thông báo thu hồi")
