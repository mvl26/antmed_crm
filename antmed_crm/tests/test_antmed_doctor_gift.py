...
class TestAntMedDoctorGift(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		cls.doctor = _ensure("AntMed Doctor", "doctor_code", "_T-GIFT-BS", {"full_name": "BS Quà", "hospital": cls.hosp})
		cls.approver = _ensure("User", "email", "_t_gift_mgr@example.com", {"first_name": "Mgr Gift", "send_welcome_email": 0})
	...
	def test_create_gift_ok(self):
		res = doctor_care.create_gift(doctor=self.doctor, item_or_text="Sách chuyên môn", value_vnd=500000, approved_by=self.approver, purpose="Tri ân hội thảo")
	...
	def test_list_gifts(self):
		doctor_care.create_gift(doctor=self.doctor, item_or_text="Hoa", value_vnd=300000, approved_by=self.approver)
