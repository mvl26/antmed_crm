...
class TestAntMedOutbox(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		cls.doctor = _ensure("AntMed Doctor", "doctor_code", "_T-OBX-BS", {"full_name": "BS Outbox", "hospital": cls.hosp})

	def test_apply_ok(self):
		res = mobile_sync.apply_outbox([{"idempotency_key": "OBX-1", "operation": "save_care_note", "payload": {"doctor": self.doctor, "content": "Ghi chú offline"}}])
	...
	def test_idempotent(self):
		op = {"idempotency_key": "OBX-IDEM", "operation": "save_care_note", "payload": {"doctor": self.doctor, "content": "x"}}
	...
	def test_reject_unknown_op(self):
		res = mobile_sync.apply_outbox([{"idempotency_key": "OBX-BAD", "operation": "frappe.delete_doc", "payload": {}}])
